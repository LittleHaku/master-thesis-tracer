...
\begin{figure}[!htbp]
    \centering
    \input{figures/flowcahrt-tracer.tex}
...
and to detect the fallback and rephrase its words
...
\begin{itemize}
  \item \textbf{General Exploration:}
    ...
    it will engage in a natural conversation by first greeting the chatbots,
    and then if the chatbots doesn't give away what it can do,
...
\begin{figure}[htpb]
  \centering
  \input{figures/functionality-node-class-diagram.tex}
  \caption{
...
and a prompt that descibes the two types of chatbots
...
\begin{itemize}
  \item \textbf{Informational chatbots:}
    ...
    \autoref{code:prompt-transactional-workflow}
    .
  ...
  \item \textbf{Transactional chatbots:}
    ...
    The prompt for transactional chatbots can be seen in \autoref{code:prompt-informational-workflow}.
\end{itemize}
...
and it is asked to reuse them to not end up with
one different category per functionality.

...
\begin{tikzpicture}[
    ...
    volume/.style={cylinder, draw, fill=gray!20, text centered, minimum height=0.2cm, minimum width=2.4cm, aspect=0.5, align=center},
...
% Title
\node[font=\Large\bfseries] at (0, 10.8) {Docker Container Architecture};

% Shared Volumes at top
\node[volume] (filevault) at (5.3, 7.0) {filevault\_data\_prod};
\node[volume] (staticfiles) at (0.3, 8.5) {static\_files\_prod};
\node[volume] (reactbuild) at (-5.3, 7.0) {react\_build};

% Top: NGINX
\node[container, fill=orange!20] (nginx) at (0, 6.2) {nginx:latest\\Port 80→8080};

% Mid: Frontend | Backend | Celery
\node[container, fill=green!20] (frontend) at (-5, 4.2) {Frontend\\React Build};
\node[container, fill=yellow!20] (backend) at (0, 4.2) {Backend\\Django API\\Port 8000};
\node[container, fill=blue!20] (celery) at (5, 4.2) {Celery Worker\\Async Tasks};

% Lower: DB | RabbitMQ
\node[container, fill=cyan!20] (db) at (-2, 2.3) {postgres:16\\Port 5432};
\node[container, fill=pink!20] (rabbitmq) at (2.5, 2.3) {rabbitmq:3-mgmt\\Port 5672};

% Volumes at bottom
\node[volume] (pgdata) at (-2, -0.5) {postgres\_data};
\node[volume] (rmqdata) at (2.5, -0.5) {rabbitmq\_data};

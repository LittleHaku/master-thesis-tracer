...
provides research works related to the thesis topics,
...
Modern conversational agents can be categorised into two main types
...
\begin{itemize}
  ...
  \item \textbf{Multi-agent programming environments:}
    ...
    they can suffer from hallucinations and higher latency
    Furthermore, their non-deterministic nature
    complicates testing, as it can cause
    conversations to deviate from their intended scope.
...
\subsection{Development Frameworks for Conversational Agents}
...
of writing software employing \acp{LLM}.
What differentiates these environments is that
there are multiple specialised \ac{AI} agents
...
where nodes are \ac{AI} agents or tools (e.g., a \ac{LLM} call or a tool)
and edges control the flow of information between the nodes.

A tool is a way of extending the capabilities of a \ac{LLM}
by enabling it to interact with external functions or \acp{API}.
Common examples include web searches, database access or

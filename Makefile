...
.PHONY: pdf
pdf:
	+latexmk -interaction=nonstopmode -outdir="$(OUT)" -lualatex -halt-on-error -jobname="$(jobname)" $(FILE)

.PHONY: watch
watch:
	+latexmk -interaction=nonstopmode -outdir="$(OUT)" -lualatex -pvc -halt-on-error -jobname="$(jobname)" $(FILE)
